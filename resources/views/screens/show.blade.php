...
<div class="container-fluid mt--6">
  <div class="modal fade" id="new-message-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true"> <div class="modal-dialog modal-dialog-centered modal-xl" role="document"> <div
    class="modal-content">
  ...
  <li class="nav-item" role="presentation">
    ...
    aria-controls="pills-home" aria-selected="true">{{ __('from database') }}</a>
    </li>
  <li class="nav-item" role="presentation">
    <a class="nav-link" id="pills-profile-tab" data-toggle="pill" href="#pills-profile" role="tab"
    aria-controls="pills-profile" aria-selected="false">{{ __('new message') }}</a>
    </li>
    </ul>
    ...
    <table class="table align-items-center table-flush">
      ...
      <tr>
        ...
        <td><a href="{{ route('assignMessage', [$screen->id, $message->id]) }}" class="btn btn-success">{{ __('Add
        Message') }}</a></td>
      </tr>
      @empty
      <tr>
...
</div> </div> <div class="modal fade" id="new-attachment-upload-modal" tabindex="-1" role="dialog"
...
<div class="modal fade" id="new-attachment-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  aria-hidden="true"> <div class="modal-dialog modal-dialog-centered modal-xl" role="document"> <div
  class="modal-content">
  <div class="modal-header">
...
<tbody>
  ...
  <th colspan="4" scope="row">
    ...
    <div class="col-xl-8"> <div class="card"> <div class="card-header border-0">
      ...
      <button type="button" data-toggle="modal" data-target="#new-attachment-upload-modal"
        class="btn btn-sm btn-primary"><i class="fa fa-plus"></i></button>
      <button type="button" data-toggle="modal" data-target="#new-attachment-modal" class="btn btn-sm btn-primary"><i
      class="fa fa-plus"></i>{{ __(' from gallery') }}</button>
      </div>
    </div>
    </div>
    <div class="card-body">
    <form class="form-inline" action="{{ route('screens.update', $screen->id) }}" method="post">
      ...
      <div class="form-group">
        ...
        <input type="checkbox" name="has_message_bar" id="hmb-value" @if($screen->has_message_bar) checked value="1"
        @endif>
      ...
      <select class="form-control mx-2" name="presentation_mode">
        <option @if($screen->presentation_mode === "standard") selected @endif value="standard">{{ __('Standard')
        }}</option>
        <option @if($screen->presentation_mode === "theater") selected @endif value="theater">{{ __('theater')
        }}</option>
      </select>
      </div>
      <div class="form-group">
      ...
      <table class="table align-items-center table-flush">
        ...
        <tbody>
          ...
          <td>
            ...
            <a href="{{ route('screen.detach', [$screen->id, $attachment->id, 'attachment']) }}" class="btn
            btn-outline-danger"><i class="fa fa-trash"></i></a>
            </div>
            </td>
            </tr>
            @empty
...
<div class="col-xl-4">
  <div class="card">
    <div class="card-header border-0">
      <div class="row align-items-center">
        ...
        <div class="col  {{ config('app.locale') === 'ar' ? 'text-left' : 'text-right' }}">
          <button data-toggle="modal" data-target="#new-message-modal" class="btn btn-sm btn-primary">{{ __('New
            Message') }}</button>
        </div>
      </div>
    </div>
      ...
      <table class="table align-items-center table-flush">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <a href="{{ route('screen.detach', [$screen->id, $message->id, 'message']) }}"
  ...
  <div class="card">
    <color-picker screen="{{ $screen->id }}"></color-picker>
  </div>
</div>
</div>
</div>
